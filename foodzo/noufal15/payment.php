...
$sql1="select * from customer where cust_id=1";
$result1=mysqli_query($link,$sql1);
$row1=mysqli_fetch_array($result1);

$sql="insert into `order` (`cust_id`, `subtotal`,`discount`,`delivery_fee`,`delivery_address`, `delivery_type`,`rest_id`) values(1,$stot,0,$row[7],'$row1[4]','$a',$id)";
mysqli_query($link,$sql);

$sqlOrd="select max(`order_id`)from `order`";
...
{ 
	$itm=$item[$i];
$sql2="SELECT product.product_id, product.product_price FROM product,menu,product_menu_map WHERE product.product_name='$itm' AND product.product_id=product_menu_map.product_id AND menu.menu_id=product_menu_map.menu_id AND menu.rest_id=$id";
$result2=mysqli_query($link,$sql2);
$row2=mysqli_fetch_array($result2);
$q=$item[$i+2];
$sql3="insert into orderproduct values ($rowOrd[0],$row2[0],$row2[1],$q)";
mysqli_query($link,$sql3);

}
// For all Products in Cart
//$sqlOrProd = "insert into `orderproduct`(`order_id`, `product_id`, `product_price`, `product_qty`) VALUES ($rowOrd[0],[value-2],[value-3],[value-4])";
//mysqli_query($link,$sqlOrProd);
